...
from drugforge.config import DEFAULT_EXHAUSTIVENESS
...
from drugforge.domain.models import ScreeningResult
from drugforge.domain.targets import get_target
from drugforge.errors import DrugForgeError, PipelineError
from drugforge.pipeline.comparison import (
    build_comparisons,
    overall_verdict,
    reference_baseline,
)
...
def _stage(name: str, action):
    ...
    try:
        return action()
    except DrugForgeError as exc:
        raise PipelineError(stage=name, cause=exc) from exc
    except Exception as exc:
        raise PipelineError(stage=name, cause=DrugForgeError(str(exc))) from exc
